...
inputs_ind.s_max = [i in [1, 2, 3, 4, 5, 6, 22, 23, 24, 25, 26, 27] ? 0 : inputs_ind.s_max[i] for i in 1:length(T)+inputs_ind.κ]
...
begin
	...
	a = groupedbar([value.(m[:Δ]) value.(m[:s])-value.(m[:g_minus])-value.(m[:b_plus]) value.(m[:b_minus]) value.(m[:g_plus]) -value.(m[:g_minus]) -value.(m[:b_plus])],
			bar_position = :stack,
			bar_width=0.7,
			xticks=(1:1:length(T)+inputs_ind.κ), #, ticklabel),
			ylims=(-20,21),
