...
begin
	...
	begin
		...
		@constraint(m, [t = 2:length(T)+inputs_ind.κ], inputs_ind.SOC_min .<= SOC[t-1] + (inputs_ind.η_plus * (b_plus[t]) + inputs_ind.η_minus * (b_minus[t] + r_b_minus[t])) * Δt / b_cap .<= inputs_ind.SOC_max)
		@constraint(m, c2_vector[i = 1:length(T)+inputs_ind.κ], r_b_minus[i] + r_Δ[i] + inputs.s_max[i] + Δ[i] + b_minus[i] - b_plus[i] - inputs.d[i] >= quantile(ξ[i], inputs_ind.p))
...
if create_plots
	...
	begin
		if termination_status(m) in [MOI.LOCALLY_SOLVED, MOI.OPTIMAL] 
			gr(size=(800,600))
			direct_solar = [inputs_ind.s_max[i] == 0.00 ? 0.00 : inputs_ind.d[i]-value.(m[:g_plus])[i]-value.(m[:b_minus])[i] for i in 1:length(T)+inputs.κ]
